...
describe('Type Tests', () => {
  test('getUserTransactions type', () => {
    ...
    expectTypeOf(transactions).toBeArray()<Transaction>();
  });
  ...
  test('categorizeTransactions type', () => {
    ...
    expectTypeOf(categorized).toBeObject<{ [key: string]: number }>();
  });

  test('calculateMonthlyAnalysis type', () => {
    const transactions = getUserTransactions(1);
    const monthlyAnalysis = calculateMonthlyAnalysis(transactions);
    expectTypeOf(monthlyAnalysis).toBeObject()<{ [key: string]: { total: number, categories: { [key: string]: number } } }>();
  });
  ...
  test('calculatePercentages type', () => {
    ...
    expectTypeOf(percentages).toBeObject()<{ [key: string]: { total: number, categories: { [key: string]: string } } }>();
  });
  ...
  test('getAnalyse type', () => {
    ...
    expectTypeOf(analysis).toBeObject()<{ [key: string]: { total: number, categories: { [key: string]: { amount: number, percentage: string } } } }>();
  });
});
