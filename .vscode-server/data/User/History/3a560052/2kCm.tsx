...
export function ModalEdit({ openModal, setOpenModal, setWizardData, setClassData, setRoomData, editedData, setEditedData, displayedData }: ModalEditProps) {

    // ----------------------------------------------------------------------------------- LocalstorageData
    const [roleToken] = useState(localStorage.getItem('roleToken') || undefined);
    const { data, error, postdata } = usePostAPI();
    const { data: putData, error: putError, putdata } = usePutAPI();
    ...
    useEffect(() => {
        ...
        setFormData({
            ...
            house_id: editedData.house_id ? editedData.house_id : 0,
            class_id: editedData.class_id ? editedData.class_id : 0,
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (editedData) {
            const route = getRouteFromDataType(displayedData);
            putdata(`${route}/${editedData.id}`, formData, roleToken);
            return;
        }

        const route = getRouteFromDataType(displayedData);
        postdata(route, formData, roleToken);
    };

    const getRouteFromDataType = (dataType: "wizards" | "subjects" | "rooms"): string => {
        switch (dataType) {
            case "wizards":
                return "wizards";
            case "subjects":
    ...
    useEffect(() => {
        ...
        switch (displayedData) {
            case "wizards":
                setWizardData((prevData: Wizard[] | undefined) => prevData ? [...prevData, data] : [data]);
                break;
            case "subjects":
                setClassData((prevData: Class[] | undefined) => prevData ? [...prevData, data] : [data]);
                break;
            case "rooms":
                setRoomData((prevData: Room[] | undefined) => prevData ? [...prevData, data] : [data]);
                break;
            default:
                break;
        }
        ...
        switch (displayedData) {
            case "wizards":
                setWizardData((prevData: Wizard[] | undefined) =>
                    prevData ? prevData.map((item: Wizard) => item.id === putData.id ? putData : item) : []
                );
                break;
            case "subjects":
                setClassData((prevData: Class[] | undefined) =>
                    prevData ? prevData.map((item: Class) => item.id === putData.id ? putData : item) : []
                );
                break;
            case "rooms":
                setRoomData((prevData: Room[] | undefined) =>
                    prevData ? prevData.map((item: Room) => item.id === putData.id ? putData : item) : []
                );
                break;
            default:
                break;
