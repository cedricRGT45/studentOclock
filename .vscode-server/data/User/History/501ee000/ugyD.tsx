...
export function Card({ data, daexport function Card({ data, dataType, is_staff, openModal, setOpenModal, setEditedData }: CardProps) {

    function handleClick() {
        setEditedData(data); // Pass the selected wizard data to ModalEdit
        setOpenModal(true); // Open the modal
    }taType, is_staff, openModal, setOpenModal, setEditedData, setWizardData, setClassData, setRoomData }: CardProps) {
    ...
    return (
        <div className="card_account_container">
            ...
            <div className="card_account_container_info">
                {dataType === "wizard" && data && (
                    <>
                        <p className="card_account_container_info_name">{(data as Wizard).firstname + ' ' + (data as Wizard).lastname}</p>
                ...
                {dataType === "class" && data && (
                    <>
                        <p className="card_account_container_info_name">{(data as Class).name}</p>
                        <div className="card_account_container_info_role">
                            <p className="card_account_container_info_role_title">Description</p>
                            <p className="card_account_container_info_role_item">{(data as Class).description}</p>
                        </div>
                    </>
                )}
                {dataType === "room" && data && (
                    <>
                        <p className="card_account_container_info_name">{(data as Room).name}</p>
                        <div className="card_account_container_info_role">
                            <p className="card_account_container_info_role_title">Capacity</p>
                            <p className="card_account_container_info_role_item">{(data as Room).capacity}</p>
                        </div>
                    </>
                )}
            </div>
            ...
            <div className={`card_dialog ${deleteModal ? 'delete_active' : ''}`}>
                <div className="card_dialog_wrapper">
                    <p>Êtes-vous sûr de supprimer {(dataType === "wizard" ? (data as Wizard).firstname + ' ' + (data as Wizard).lastname : data.name)} ?</p>
                    <div className="card_dialog_wrapper_button">
                        <button onClick={() => setDeleteModal(false)}>Retour</button>
                        <button className="button_remove" onClick={() => handleDelete(data.id)}>Supprimer</button>
                    </div>
                </div>
            </div>
        </div>
