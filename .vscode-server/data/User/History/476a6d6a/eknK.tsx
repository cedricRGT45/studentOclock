...
export function Account({ header }: AccountProps) {
    ...
    const [classesData, setClassesData] = useState<Class[] | undefined>(undefined)
    ...
    const { data: classes, error: classesError } = useFetchAPI<Class[]>({ route: 'subjects', token: roleToken })
    const { data: rooms, error: roomsError } = useFetchAPI<Room[]>({ route: 'rooms', token: roleToken })

    useEffect(() => {
        if (wizardsError || classesError || roomsError) {
            toast.error('Une erreur est survenue', {
                description: 'Impossible de charger les données',
            })
        }
    }, [wizardsError, classesError, roomsError])
